...
Your text ("Hello") gets converted into raw binary (0s and 1s), because computers and signals only understand numbers.

### Step 3: QAM Modulation (Bits → Points on a Map)
...
### Step 4: OFDM — Spread the Data Across Many Frequencies
Instead of putting all data on one single tone, AirBudds spreads it across **hundreds of tiny frequency channels** simultaneously — like playing a chord on a piano instead of a single key. This is called **OFDM** (Orthogonal Frequency Division Multiplexing), and it makes the signal extremely robust.
...
### Step 5: Add a Sync Chirp (Preamble)
A quick **chirp** (a sound that sweeps from low pitch to high pitch) is placed at the very beginning. This chirp acts as a "start marker" — when the receiver hears it, it knows exactly where the data begins.

### Step 6: Play the Sound! 🔊
The final audio waveform is played through the speaker (or saved to a `.wav` file). What you hear is a short chirp followed by a burst of buzzy noise — that "noise" is actually your data!
...
### Step 1: Find the Start (Sync)
The receiver scans the recorded audio for the **chirp preamble** using a technique called "matched filtering." It slides the known chirp pattern across the recording and finds where the match is strongest — that's where the data starts.
...
### Step 5: QAM Demodulation (Points → Bits)
...
Finally, the recovered bits are re-assembled into bytes, and the CRC checksum is verified. If it matches → ✅ the data is perfect. If it doesn't → ⚠️ something went wrong (too much noise, bad microphone, etc.).
...
3. On **Computer A** (sender): Click **"Transmit Text (Speaker)"**.
4. On **Computer B** (receiver): Click **"Start Listening"** *before* the sender transmits.
5. The receiver's microphone picks up the sound, decodes it, and shows the result in **"Decoded RX Result"**.
...
│                         TRANSMIT SIDE                               │
│                                                                     │
│   "Hello!"  →  CRC  →  Bits  →  QAM Map  →  OFDM (IFFT + CP)     │
│       →  Chirp Prepend  →  🔊 Speaker / 💾 WAV File                │
└─────────────────────────────────────────────────────────────────────┘
                              ↓  🔈  sound waves  ↓
┌─────────────────────────────────────────────────────────────────────┐
│                         RECEIVE SIDE                                │
│                                                                     │
│   🎤 Mic / 📂 WAV Upload  →  Chirp Sync  →  AGC  →  FFT           │
│       →  Pilot Channel Est.  →  Equalize  →  QAM Demap  →  CRC    │
│       →  "Hello!"  ✅                                               │
└─────────────────────────────────────────────────────────────────────┘
```
